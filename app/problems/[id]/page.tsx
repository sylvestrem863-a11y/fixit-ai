...
export default async function ProblemDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: actions } = await supabase.from('action_items').select('*').eq('problem_id', id).order('position')
  ...
  return (
    <main className="container">
      ...
      <section className="dashboard-head">
        ...
        <div className="problem-meta"><span className={`priority priority-${problem.priority}`}>{problem.priority}</span><span>{problem.status}</span></div>
      </section>
      ...
      <section className="card detail-section">
        ...
        <div className="action-list">
          {(actions?.length ? actions : (analysis.recommended_actions || []).map((a: any, i: number) => ({ id: `ai-${i}`, title: a.title, description: a.description, completed: false }))).map((action: any) => (
            <div className="action-item" key={action.id}>
              <input type="checkbox" checked={!!action.completed} readOnly aria-label={action.title} />
              <div><strong>{action.title}</strong>{action.description && <p className="muted">{action.description}</p>}</div>
            </div>
          ))}
        </div>
      </section>

      {(analysis.warnings?.length || analysis.documents_to_keep?.length || analysis.required_information?.length) ? (
